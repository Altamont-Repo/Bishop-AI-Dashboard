...
export function NewOrderModal({ onClose }: { onClose: () => void }) {
  ...
  const [itemNumber, setItemNumber] = useState(ds.items[0]?.itemNumber ?? "");
  const [qty, setQty] = useState(10);
  const [orderType, setOrderType] = useState<OrderType>("Customer");
  const [importance, setImportance] = useState<Importance>("Medium");
  const [neededBy, setNeededBy] = useState(toISO(addDays(new Date(today), 7)));
  ...
  const submit = () => {
    ...
    addOrder({ locationId, productionNo: productionNo.trim(), itemNumber, qtyNeeded: qty, orderType, importance, neededBy, value });
    onClose();
  };
  ...
  return (
    <Modal
      ...
      footer={<>
        ...
        <button className="btn primary" onClick={submit} disabled={!productionNo.trim() || !item}>Create order</button>
      </>}
    >
...
export function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label style={{ display: "block", fontSize: 12 }}>
      <div style={{ color: "var(--ink2)", marginBottom: 4, fontWeight: 600 }}>{label}</div>
      {children}
    </label>
  );
}
